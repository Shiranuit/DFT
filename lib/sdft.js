...
function prepareDownload(socket, info) {
  ...
  socket.on('close', () => {
    ...
    progressbar.stop();
    console.log('Download compressed file');
...
function compressFile(file, callback) {
  console.log(`Start compressing file: ${file}`);
...
function compressDirectory(file, callback) {
  console.log(`Start compressing directory: ${file}`);
  const zip = zipArchiver.directory(file, false);
  const writeStream = fs.createWriteStream(`./${path.basename(file)}.zip`);
  zip.pipe(writeStream);
  zip.finalize();
  zip.on('end', () => {
    console.log('Directory compression complete');
    callback();
  });
}
...
const socket = tls.connect({
  ...
  port: config.client.port || args.port || 9966,
